...
namespace System.Reflection
{
    internal sealed partial class RuntimeConstructorInfo : ConstructorInfo
    {
            ...
            CultureInfo? culture)
        {
            ...
            if (argCount == 0)
            {
                Invoker.InvokeConstructorWithoutAlloc(obj, default, invokeAttr);
            }
            else
            {
                Debug.Assert(parameters is not null);

                MethodInvoker.StackAllocatedObjects stackArgStorage = default;
                scoped Span<object?> copyOfParameters;

                if (argCount <= MethodInvoker.MaxStackAllocArgCount)
                {
                    copyOfParameters = stackArgStorage._args.AsSpan(argCount);
                }
                else
                {
                    object?[] heapArgStorage = new object?[argCount];
                    copyOfParameters = new(ref heapArgStorage[0], argCount);
                }
                ...
                Invoker.InvokeConstructorWithoutAlloc(obj, copyOfParameters, invokeAttr);
