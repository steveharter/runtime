...
namespace System.Reflection
{
    internal sealed partial class RuntimeConstructorInfo : ConstructorInfo
    {
            ...
            CultureInfo? culture)
        {
            ...
            if (argCount == 0)
            {
                retValue = Invoker.InvokeConstructorWithoutAlloc(obj, default, invokeAttr);
            }
            else
            {
                Debug.Assert(parameters is not null);

                StackAllocatedObjects _stackAllocatedObjects = default;
                object?[]? _heapAllocatedObjects;
                scoped Span<object?> copyOfParameters;

                if (argCount <= MethodInvoker.MaxStackAllocArgCount)
                {
                    copyOfParameters = new(ref _stackAllocatedObjects._arg0, argCount);
                }
                else
                {
                    _heapAllocatedObjects = new object?[argCount];
                    copyOfParameters = new(ref _heapAllocatedObjects[0], argCount);
                }

                bool hasTypeMissing = CheckArguments(
                    parameters,
                    copyOfParameters,
                    ArgumentTypes,
                    binder,
                ...
                retValue = Invoker.InvokeConstructorWithoutAlloc(obj, copyOfParameters, invokeAttr);
        ...
        public override object Invoke(BindingFlags invokeAttr, Binder? binder, object?[]? parameters, CultureInfo? culture)
        {
            ...
            if (argCount == 0)
            {
                retValue = Invoker.InvokeNoParams(obj: null, invokeAttr)!;
            }
            else
            {
                retValue = InvokeWithArgs(invokeAttr, binder, parameters!, culture);
            }

            return retValue;
        }

        // This is a separate method for better performance of the zero-parameter case.
        private object InvokeWithArgs(
            BindingFlags invokeAttr,
            Binder? binder,
            object?[] parameters,
            CultureInfo? culture)
        {
            ...
            object retValue = Invoker.Invoke(obj: null, copyOfParameters, invokeAttr)!;
